...
const kickFinale = new Tone.Part(((time, note) => {
    kick.triggerAttackRelease(note, '16n', time);
}), [["0:0:0", "C2"], ["0:1:0", "C2"], ["0:2:0", "C2"], ["0:3:0", "C2"]]);
kickFinale.loop = false;
...
const snareFinale = new Tone.Part(((time) => {
    snare.triggerAttack(time);
...
const guitarFinale = new Tone.Part(((time, chord) => {
    guitar.triggerAttackRelease(chord, "32n", time);
}), [["0:0:0", gChord],
["0:1:0", gChord],
["0:2:0", gChord],
...
guitarFinale.loop = false;
guitarFinale.loopEnd = "1m";
guitarFinale.humanize = true;
...
const bassFinale = new Tone.Sequence(((time, note) => {
    bass.triggerAttackRelease(note, "16n", time);
}), [["C2", "D2", "E2", "F2"], ["G2", "A2", "B2", "C3"], ["D3", "E3", "F3", "G3"], "C4"], "4n");
bassFinale.loop = false;
...
const fluteFinale = new Tone.Sequence(((time, note) => {
    flute.triggerAttackRelease(note, "16n", time);
}), [["G4", "B4"], ["C5", "D5"], ["E5", "B5"], "C6"], "4n");

fluteFinale.loop = false;
